...
object SecurityManager {
    ...
    fun checkRootStatus(context: Context, onCompromised: (() -> Unit)? = null) {
        coroutineScope.launch {
            ...
            // Placeholder for Play Integrity API integration (always false here)
            val playIntegrityCompromised = false // TODO: Implement real check

            if (rootDetected || playIntegrityCompromised) {
                //Timber.w(TAG, "Security compromise detected!")
                withContext(Dispatchers.Main) {
                    isCompromised.value = true
                    onCompromised?.invoke()
                }
            } else {
                //Timber.i(TAG, "Device passing security checks.")
                withContext(Dispatchers.Main) {
